...
export const QASettings: React.FC = () => {
  ...
  return (
    <div className="tw-space-y-4">
      <section>
        <div className="tw-space-y-4">
          {/* Use Smart Connections for search */}
          <SettingItem
            type="switch"
            title="Use Smart Connections"
            description="When enabled, delegates semantic search to the Smart Connections plugin instead of using the built-in embedding system. Requires Smart Connections to be installed and enabled. Takes priority over built-in semantic search."
            checked={settings.useSmartConnections}
            onCheckedChange={(checked) => updateSetting("useSmartConnections", checked)}
          />

          {/* Built-in embedding/index controls - hidden when Smart Connections is active */}
          ...
          {/* Built-in embedding/index settings - hidden when SC handles search */}
          {!settings.useSmartConnections && (
            <>
              {/* Embedding Model */}
              <SettingItem
                  ...
                  <div className="tw-space-y-2">
                    <div className="tw-flex tw-items-center tw-gap-1.5">
                      <span className="tw-font-medium tw-leading-none tw-text-accent">
                        Powers Semantic Vault Search and Relevant Notes. Enable Semantic Search to
                        use it.
                      </span>
                      <HelpTooltip
                        content={
                          <div className="tw-flex tw-max-w-96 tw-flex-col tw-gap-2">
                            <div className="tw-pt-2 tw-text-sm tw-text-muted">
                              ...
                              semantic search and Question Answering (QA) functionality. Changing
          ...
          <SettingItem
            type="slider"
            title="Max Sources"
            description="Hendrik goes through your vault to find relevant notes and passes the top N to the LLM. Default for N is 15. Increase if you want more notes included in the answer generation step."
            min={1}
            max={128}
            step={1}
          ...
          {/* Embedding-related settings - Only shown when semantic search is enabled and SC is off */}
          {!settings.useSmartConnections && settings.enableSemanticSearchV3 && (
            <>
              ...
              <SettingItem
                ...
                description="Number of partitions for Hendrik index. Default is 1. Increase if you have issues indexing large vaults. Warning: Changes require clearing and rebuilding the index!"
          ...
          {/* Lexical Search RAM Limit - only relevant when SC is off */}
          {!settings.useSmartConnections && (
            <SettingItem
              type="slider"
          ...
          {/* Enable Folder and Graph Boosts - only relevant when SC is off */}
          {!settings.useSmartConnections && (
            <SettingItem
              type="switch"
          ...
          {/* Built-in index storage settings - hidden when SC is active */}
          {!settings.useSmartConnections && (
            <>
              ...
              <SettingItem
                ...
                description="If enabled, store the semantic index in .obsidian so it syncs with Obsidian Sync. If disabled, store it under .hendrik/ at the vault root."
